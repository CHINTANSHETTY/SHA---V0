...
from crypto.engine.decrypt import decrypt_payload as legacy_kdr_decrypt_payload
from crypto.models.package import EncryptedPackage
from crypto.models.exceptions import CryptoError, AuthenticationError
from decrypt import decryptRecord as legacy_v0_decryptRecord
...
def encryptPage():
    ...
    if request.method == "POST":
        ...
        try:
            # Encrypt using Revised SHA-512 2x2 Margolus CA Cryptosystem
            cipherText = encrypt_payload(patientData, password)
            ...
            message = "Patient record encrypted and saved successfully with Revised SHA-512 2x2 Margolus CA."
...
def decryptPage(recordId):
    ...
    try:
        ...
        if cipherText.startswith(REVISED_PREFIX):
            originalData = decrypt_payload(cipherText, password)
        elif cipherText.startswith("{") and "KDR-CA-AEAD" in cipherText:
            pkg = EncryptedPackage.from_json(cipherText)
            originalData = legacy_kdr_decrypt_payload(pkg, password)
        else:
            try:
                originalData = decrypt_payload(cipherText, password)
            except Exception:
                originalData = legacy_v0_decryptRecord(cipherText, password)
...
def updatePatient(recordId):
    ...
    try:
        newCipherText = encrypt_payload(patientData, password)
